"""
MAMBA Diffusion V3 - Spatial Ordering with Morton Curves

Key improvement over V1:
- Morton (Z-order) curve sequencing for better spatial locality
- MAMBA processes spatially coherent sequences
- Neighboring pixels in 2D are also neighbors in 1D
- Same architecture as V1, just better ordering

Expected improvements:
- Better spatial coherence from improved locality
...
v3_dir = os.path.dirname(script_dir)  # v3/
repo_root = os.path.dirname(v3_dir)   # MambaFlowMatching/
...
from core.spatial_ordering import get_morton_order_indices
...
class MAMBADiffusion(nn.Module):
    """
    MAMBA Diffusion V3 - with Morton curve spatial ordering

    Key improvements over V1:
    - Morton (Z-order) curve for better spatial locality
    - Neighboring pixels in 2D are also neighbors in 1D sequence
    - Better state propagation through MAMBA

    Architecture:
    - Same as V1 (6 unidirectional MAMBA layers)
    - Just reorder sequences using Morton curves before/after MAMBA
    """
    def __init__(
        self,
        ...
        d_state=16,
        dropout=0.1,
        use_morton_ordering=True,  # NEW: Enable Morton curve ordering
        image_resolution=32  # NEW: Resolution for Morton curve
    ):
        super().__init__()
        self.d_model = d_model
        self.use_morton_ordering = use_morton_ordering
        self.image_resolution = image_resolution

        # Precompute Morton ordering indices
        if self.use_morton_ordering:
            morton_indices = get_morton_order_indices(image_resolution)
            self.register_buffer('morton_indices', torch.from_numpy(morton_indices).long())
            # Inverse indices to restore original order
            inverse_indices = torch.argsort(self.morton_indices)
            self.register_buffer('inverse_morton_indices', inverse_indices)
    ...
    def forward(self, noisy_values, query_coords, t, input_coords, input_values):
        ...
        # NEW: Apply Morton ordering to query tokens (if enabled and full grid)
        # Only reorder if we have full resolution grid (N_out == resolution^2)
        if self.use_morton_ordering and N_out == self.image_resolution ** 2:
            query_tokens = query_tokens[:, self.morton_indices, :]
            # Note: input tokens are sparse, so we don't reorder them
        ...
        # NEW: Restore original ordering after MAMBA processing
        if self.use_morton_ordering and N_out == self.image_resolution ** 2:
...
def train_flow_matching(
    ...
    visualize_every=50, eval_every=10, save_every=10, save_dir='checkpoints_mamba_v3'
):
    """
...
def main():
    ...
    parser.add_argument('--save_dir', type=str, default='checkpoints_mamba_v3', help='Save directory')
    ...
    # Initialize model
    print("=" * 60)
    print("MAMBA DIFFUSION V3 - Morton Curve Ordering")
    print("=" * 60)
    print("Key improvements over V1:")
    print("  ✓ Morton (Z-order) curve for better spatial locality")
    print("  ✓ Neighboring pixels in 2D are neighbors in 1D")
    print("  ✓ Better state propagation through MAMBA")
    print("  ✓ Same architecture complexity as V1")
    print("=" * 60)
    print("\nInitializing model...")
    ...
    print(f"Morton ordering: {'ENABLED' if model.use_morton_ordering else 'DISABLED'}")
    print(f"Image resolution: {model.image_resolution}×{model.image_resolution}")
